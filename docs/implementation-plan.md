...
### Step 4E.4 — run drawer and run-state motion

Implement only the run UI against the existing execution contracts and API.
Do not begin Step 4E.5 or Step 5.

1. Add typed frontend clients for run create/open/list/save/recover, SSE event
   replay, cancellation, and review decisions. Preserve schema fields and map
   HTTP/SSE failures into actionable researcher-facing messages.
2. Add a run drawer that shows the aggregate run state, every node state and
   attempt, cache reuse, ordered events, failures, pending reviews, and artifact
   metadata. Artifact actions must use existing authorized project paths and
   must not expose arbitrary filesystem browsing.
3. Connect `Run workflow` to validation and the authorized open project. Keep
   the editor workflow stable during execution and prevent duplicate launches.
4. Implement reconnect from the last received event sequence without duplicate
   rendering. Opening or recovering a run must converge on the persisted record
   even when SSE disconnects or events arrive around the initial snapshot.
5. Add explicit review approve/reject and cancel controls with pending/disabled
   states, idempotent response handling, and clear terminal outcomes.
6. Animate discrete run/node/drawer state changes without animating continuous
   pointer tracking. Honor `prefers-reduced-motion` for every new transition.
7. Add focused client and component tests for success, actionable failure,
   review/resume, cancellation, cache reuse, artifacts, duplicate launch guards,
   out-of-order snapshot/event timing, reconnect/replay, and reduced motion.
8. Run the complete gate and live browser probes for the demonstration fixture:
   successful execution and artifacts, second-run reuse, failure, review/resume,
   cancellation, and SSE reconnect. Record exact evidence and request monitoring
   review before committing.

Leave the run-drawer and reduced-motion checkboxes unchecked for the monitor.
Step 4E.5 performs the complete Step 4 gate. Step 5A begins only after Step 4 is
approved.
